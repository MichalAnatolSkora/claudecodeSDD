...
- `guides/spec-plan-tasks-guide.md` — **the flagship guide and centerpiece of the repo: the core spec → plan → tasks loop.** Two full worked-example trios (a non-trivial feature as the three-file trio + a small feature as a one-file trio), six AI-authoring prompts per artifact, iteration loops, cross-artifact consistency checks, slash-command sketches. Deliberately kept simple (no toolkit/scaffolder). Keep this the most polished guide.
